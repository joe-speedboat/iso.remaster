...
test -f $BASE_DIR/inject/ks-custom_rocky10.cfg || ( echo kickstart not found ; exit 1)
test -f $BASE_DIR/inject/ks_efi-custom_rocky10.cfg || ( echo kickstart not found ; exit 1)
...
grep -q "ks_efi-custom_rocky10.cfg" $BASE_DIR/iso-remaster/EFI/BOOT/grub.cfg 
if [ $? -ne 0 ]
then
  echo "inject efi setup menu entry"
  awk -v proj="$PROJ_NAME" -v iso="$ISO_NAME" -v proj_cfg="ks_efi-custom_rocky10.cfg" 'flag==0 && /^menuentry / {print "menuentry '\''"proj"'\'' --class fedora --class gnu-linux --class gnu --class os {\n        linuxefi /images/pxeboot/vmlinuz inst.stage2=hd:LABEL=''"iso"'' inst.ks=cdrom:/inject/''"proj_cfg"''\n        initrdefi /images/pxeboot/initrd.img\n}\n"; print; flag=1; next} 1' $BASE_DIR/iso-remaster/EFI/BOOT/grub.cfg > $BASE_DIR/temp && mv -f $BASE_DIR/temp $BASE_DIR/iso-remaster/EFI/BOOT/grub.cfg
else
