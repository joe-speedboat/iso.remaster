...
test -f $BASE_DIR/inject/ks_efi-custom_rhel9.cfg || ( echo kickstart not found ; exit 1)
...
grep -q 'label linux-custom_rhel9' $BASE_DIR/iso-remaster/isolinux/isolinux.cfg 
if [ $? -ne 0 ]
then
  echo "insert boot loader entry for legacy bios"
  sed -i "/^label linux/i label linux-custom_rhel9\n  menu label ^$PROJ_NAME\n  kernel vmlinuz\n  append initrd=initrd.img inst.stage2=hd:LABEL=$ISO_NAME inst.ks=cdrom:/inject/ks-custom_rhel9.cfg\n" $BASE_DIR/iso-remaster/isolinux/isolinux.cfg
else
